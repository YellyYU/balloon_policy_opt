...
class libvirtInterface(HypervisorInterface):
    ...
    def getVmList(self, guestnum):
        #try:
        #    dom_list = self.conn.listDomainsID()
        #except libvirt.libvirtError, e:
        #    self._handleException(e)
        #    return []
        tmp = os.popen('pwd').readlines()
        #print tmp
        #return [4444]
        return range(0,guestnum)
        #return dom_list

    def getVmInfo(self, id):
        data = {}
        #guest_domain = self._getDomainFromID(id)
        data['uuid'] = 4444+id #self._domainGetUUID(guest_domain)
        data['name'] = "ddtest"+str(id)#self._domainGetName(guest_domain)
        data['pid'] =  0 #self._domainGetPid(data['uuid']), we donot need crare
    ...
    def startVmMemoryStats(self, uuid):
        ...
        tmp = os.popen('/home/yelly/qemu-2.8.0/scripts/qmp/qmp-dd-shell start_stats localhost:'+str(uuid)).readlines()
        #print tmp
        pass
    ...
    def getVmMemoryStats(self, uuid):
        ...
        #return ret
        #print uuid
        #tmp = os.popen('/home/yelly/qemu-2.8.0/scripts/qmp/qmp-dd-shell show_stats localhost:'+'4444').readline()
        tmp = os.popen('/home/yelly/qemu-2.8.0/scripts/qmp/qmp-dd-shell show_stats localhost:'+str(uuid)).readline()
        tmp_dict = json.loads(tmp)
        #print tmp
        #print tmp_dict['return']['stats']['stat-free-memory']
	# stat-total-memory - MemTotal: Assigned guest memory minus memory reserved for 
	#				firmware/BIOS and kernel itself. The remaining
	#				part (MemTotal) is what can be allocated by kernel.
	# stat-free-memory - MemFree: Memory that have not been used by system.
	#				(MemTotal-MemFree) is what have been used. 
	# stat-available-memory - MemAvailable: MemFree plus other recyclable memory,
	#				e.g. cache/buffer, slab.
	#				This value is estimated and not precise.
	#				It represent the actual memory that could be
	#				used on demand.
	# The returned memory values are in unit of Byte!
        ret['mem_available'] = tmp_dict['return']['stats']['stat-total-memory'];
        ret['mem_usable'] = tmp_dict['return']['stats']['stat-available-memory'];
        ret['mem_unused'] = tmp_dict['return']['stats']['stat-free-memory'];
        ret['major_fault'] = tmp_dict['return']['stats']['stat-major-faults'];
        ret['minor_fault'] = tmp_dict['return']['stats']['stat-minor-faults'];
        ret['swap_in'] = tmp_dict['return']['stats']['stat-swap-in'];
        ret['swap_out'] = tmp_dict['return']['stats']['stat-swap-out'];
	#print 'getVmMemoryStats, ret:', ret
        return ret
    ...
    def getVmBalloonInfo(self, uuid):
        ...
        #return ret
        tmp = os.popen('/home/yelly/qemu-2.8.0/scripts/qmp/qmp-dd-shell query-balloon localhost:'+str(uuid)).readline()
        tmp_dict = json.loads(tmp)
        #print tmp
        #print tmp_dict['return']['actual']
        # unit: Byte
        # balloon_max: 256MB, balloon_min: 50MB
        #print "getVmBalloonInfo, balloon_cur: int(tmp_dict['return']['actual']=%d" % int(tmp_dict['return']['actual'])
        ret = {'balloon_max':268435456,'balloon_cur':int(tmp_dict['return']['actual']),
                'balloon_min':52428800}
        return ret
        #return None
    ...
    def setVmBalloonTarget(self, uuid, target):
        ...
        tmp = os.popen('/home/yelly/qemu-2.8.0/scripts/qmp/qmp-dd-shell balloon localhost:'+str(uuid)+' '+str(target)).readline()
        tmp_dict = json.loads(tmp)
