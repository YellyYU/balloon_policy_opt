...
outputfile=/home/yelly/balloon_policy_opt/analysis/analysis_result_pair_opt_slop_p100_125_$(date +'%m%d-%H%M').csv
# delete existing output file
rm -f $outputfile

for policy in `ls $result_dir`
do
	if echo $policy  | grep 'opt'
	then
		echo "*********** $policy ***********" >> $outputfile
		echo "*********** $policy ***********"
		for condition in `ls $result_dir/$policy`
		do
			if echo $condition   | grep 'mix_workload_slop_p40_125'
			then
				...
				if [ "$finished" -ne 1 ]
				then
					echo "$result_dir/$policy/$condition/result_sup_001.txt not finished!!!" >> $outputfile
					echo "$result_dir/$policy/$condition/result_sup_001.txt not finished!!!"
				fi
