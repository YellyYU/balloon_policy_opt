...
#policy_map=("dynamic_memory_allocation_controller" "U-tube" "Hyper-V-dynamic-memory" "Wei-Zhe_Zhang" "iballoon" "xenserver-dynamic-memory-control" "kvm_autoballoon" "ovirt-balloon-qos")
policy_map=("opt" "no-policy")
condition_map=("mix_workload_p40" "mix_workload_slop_p100")

for policy in ${policy_map[@]}; do
	for condition in ${condition_map[@]}; do
		outputfile="/home/yelly/balloon_policy_opt/analysis/analysis_pair_time_"${policy}"_"${condition}"150_"$(date '+%m%d-%H%M')".csv"
